#########################################################################
# File Name: android_build_aac.sh
# Author: liuwch
# mail: liuwenchang1234@163.com
# Created Time: 六  6/22 20:44:42 2019
#########################################################################
#!/bin/bash
...
API=23
extra_cflags=(
    "-march=armv5te -msoft-float -D__ANDROID__  -D__ANDROID_API__=$API -D__ARM_ARCH_5TEJ__"
...
PROJECT=fdkaac
SOURCE_PATH="$ROOT/fdk-aac-2.0.0"
...
do
    ...
    # use clang++, not g++, because x86/x86_64 error can't find stddef.h
	export CXX="${cross_prefix[i]}clang++"
	export CC="${cross_prefix[i]}clang"
